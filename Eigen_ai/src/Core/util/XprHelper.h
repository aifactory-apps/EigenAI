...
#ifndef EIGEN_AI_XPRHELPER_H
#define EIGEN_AI_XPRHELPER_H

// just a workaround because GCC seems to not really like empty structs
// FIXME: gcc 4.3 generates bad code when strict-aliasing is enabled
// so currently we simply disable this optimization for gcc 4.3
#if EIGEN_AI_COMP_GNUC && !EIGEN_AI_GNUC_AT(4,3)
  #define EIGEN_AI_EMPTY_STRUCT_CTOR(X) \
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE X() {} \
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE X(const X& ) {}
#else
  #define EIGEN_AI_EMPTY_STRUCT_CTOR(X)
#endif
...
{
  public:
    EIGEN_AI_EMPTY_STRUCT_CTOR(variable_if_dynamic)
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE explicit variable_if_dynamic(T v) { EIGEN_AI_ONLY_USED_FOR_DEBUG(v); eigen_assert(v == T(Value)); }
    EIGEN_AI_DEVICE_FUNC static EIGEN_AI_STRONG_INLINE T value() { return T(Value); }
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE void setValue(T) {}
};

template<typename T> class variable_if_dynamic<T, Dynamic>
{
    T m_value;
    EIGEN_AI_DEVICE_FUNC variable_if_dynamic() { eigen_assert(false); }
  public:
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE explicit variable_if_dynamic(T value) : m_value(value) {}
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE T value() const { return m_value; }
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE void setValue(T value) { m_value = value; }
};
...
{
  public:
    EIGEN_AI_EMPTY_STRUCT_CTOR(variable_if_dynamicindex)
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE explicit variable_if_dynamicindex(T v) { EIGEN_AI_ONLY_USED_FOR_DEBUG(v); eigen_assert(v == T(Value)); }
    EIGEN_AI_DEVICE_FUNC static EIGEN_AI_STRONG_INLINE T value() { return T(Value); }
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE void setValue(T) {}
};
...
{
    T m_value;
    EIGEN_AI_DEVICE_FUNC variable_if_dynamicindex() { eigen_assert(false); }
  public:
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE explicit variable_if_dynamicindex(T value) : m_value(value) {}
    EIGEN_AI_DEVICE_FUNC T EIGEN_AI_STRONG_INLINE value() const { return m_value; }
    EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE void setValue(T value) { m_value = value; }
};
...
template<int ArrayBytes, int AlignmentBytes,
         ...
         bool TryHalf   =  bool(EIGEN_AI_MIN_ALIGN_BYTES<AlignmentBytes) >
...
template<typename T, int Size> struct compute_default_alignment {
  enum { value = compute_default_alignment_helper<Size*sizeof(T),EIGEN_AI_MAX_STATIC_ALIGN_BYTES>::value };
};

template<typename T> struct compute_default_alignment<T,Dynamic> {
  enum { value = EIGEN_AI_MAX_ALIGN_BYTES };
};
...
template<typename _Scalar, int _Rows, int _Cols,
         int _Options = AutoAlign |
                          ...
                          : EIGEN_AI_DEFAULT_MATRIX_STORAGE_ORDER_OPTION ),
         int _MaxRows = _Rows,
         int _MaxCols = _Cols
...
{
  enum { diag_size = EIGEN_AI_SIZE_MIN_PREFER_DYNAMIC(ExpressionType::RowsAtCompileTime, ExpressionType::ColsAtCompileTime),
         max_diag_size = EIGEN_AI_SIZE_MIN_PREFER_FIXED(ExpressionType::MaxRowsAtCompileTime, ExpressionType::MaxColsAtCompileTime)
  };
...
#define EIGEN_AI_CHECK_BINARY_COMPATIBILIY(BINOP,LHS,RHS) \
  EIGEN_AI_STATIC_ASSERT((Eigen_ai::internal::has_ReturnType<ScalarBinaryOpTraits<LHS, RHS,BINOP> >::value), \
    YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
    
} // end namespace Eigen_ai

#endif // EIGEN_AI_XPRHELPER_H
