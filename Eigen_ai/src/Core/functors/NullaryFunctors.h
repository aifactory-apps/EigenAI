...
#ifndef EIGEN_AI_NULLARY_FUNCTORS_H
#define EIGEN_AI_NULLARY_FUNCTORS_H
...
struct scalar_constant_op {
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE scalar_constant_op(const scalar_constant_op& other) : m_other(other.m_other) { }
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE scalar_constant_op(const Scalar& other) : m_other(other) { }
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE const Scalar operator() () const { return m_other; }
  template<typename PacketType>
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE const PacketType packetOp() const { return internal::pset1<PacketType>(m_other); }
...
template<typename Scalar> struct scalar_identity_op {
  EIGEN_AI_EMPTY_STRUCT_CTOR(scalar_identity_op)
  template<typename IndexType>
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE const Scalar operator() (IndexType row, IndexType col) const { return row==col ? Scalar(1) : Scalar(0); }
};
template<typename Scalar>
...
struct linspaced_op_impl<Scalar,Packet,/*IsInteger*/false>
{
  ...
  template<typename IndexType>
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE const Scalar operator() (IndexType i) const {
  ...
  template<typename IndexType>
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE const Packet packetOp(IndexType i) const
  {
    // Principle:
...
template <typename Scalar, typename PacketType> struct linspaced_op
{
  ...
  template<typename IndexType>
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE const Scalar operator() (IndexType i) const { return impl(i); }

  template<typename Packet,typename IndexType>
  EIGEN_AI_DEVICE_FUNC EIGEN_AI_STRONG_INLINE const Packet packetOp(IndexType i) const { return impl.packetOp(i); }
...
#if !( (EIGEN_AI_COMP_MSVC>1600) || (EIGEN_AI_GNUC_AT_LEAST(4,8)) || (EIGEN_AI_COMP_ICC>=1600))
...
#endif // EIGEN_AI_NULLARY_FUNCTORS_H
