...
#ifndef EIGEN_AI_GENERAL_MATRIX_VECTOR_H
#define EIGEN_AI_GENERAL_MATRIX_VECTOR_H
...
EIGEN_AI_DONT_INLINE static void run(
...
template<typename Index, typename LhsScalar, typename LhsMapper, bool ConjugateLhs, typename RhsScalar, typename RhsMapper, bool ConjugateRhs, int Version>
EIGEN_AI_DONT_INLINE void general_matrix_vector_product<Index,LhsScalar,LhsMapper,ColMajor,ConjugateLhs,RhsScalar,RhsMapper,ConjugateRhs,Version>::run(
  Index rows, Index cols,
  const LhsMapper& lhs,
  const RhsMapper& rhs,
        ResScalar* res, Index resIncr,
  RhsScalar alpha)
{
  EIGEN_AI_UNUSED_VARIABLE(resIncr);
  eigen_internal_assert(resIncr==1);
  #ifdef _EIGEN_AI_ACCUMULATE_PACKETS
  #error _EIGEN_AI_ACCUMULATE_PACKETS has already been defined
  #endif
  #define _EIGEN_AI_ACCUMULATE_PACKETS(Alignment0,Alignment13,Alignment2) \
    pstore(&res[j], \
...
//  const Index PeelAlignedMask = ResPacketSize*peels-1;
        ...
        switch(alignmentPattern)
        {
          case AllAligned:
            for (Index j = alignedStart; j<alignedSize; j+=ResPacketSize)
              _EIGEN_AI_ACCUMULATE_PACKETS(Aligned,Aligned,Aligned);
            break;
          case EvenAligned:
            for (Index j = alignedStart; j<alignedSize; j+=ResPacketSize)
              _EIGEN_AI_ACCUMULATE_PACKETS(Aligned,Unaligned,Aligned);
            break;
          case FirstAligned:
          {
            ...
            for (; j<alignedSize; j+=ResPacketSize)
              _EIGEN_AI_ACCUMULATE_PACKETS(Aligned,Unaligned,Unaligned);
            break;
          }
          default:
            for (Index j = alignedStart; j<alignedSize; j+=ResPacketSize)
              _EIGEN_AI_ACCUMULATE_PACKETS(Unaligned,Unaligned,Unaligned);
            break;
        }
      }
  ...
  #undef _EIGEN_AI_ACCUMULATE_PACKETS
}
...
template<typename Index, typename LhsScalar, typename LhsMapper, bool ConjugateLhs, typename RhsScalar, typename RhsMapper, bool ConjugateRhs, int Version>
EIGEN_AI_DONT_INLINE void general_matrix_vector_product<Index,LhsScalar,LhsMapper,RowMajor,ConjugateLhs,RhsScalar,RhsMapper,ConjugateRhs,Version>::run(
  Index rows, Index cols,
  const LhsMapper& lhs,
  const RhsMapper& rhs,
  ResScalar* res, Index resIncr,
  ResScalar alpha)
{
  ...
  {
    // FIXME: what is the purpose of this EIGEN_AI_ALIGN_DEFAULT ??
    EIGEN_AI_ALIGN_MAX ResScalar tmp0 = ResScalar(0);
...
#endif // EIGEN_AI_GENERAL_MATRIX_VECTOR_H
