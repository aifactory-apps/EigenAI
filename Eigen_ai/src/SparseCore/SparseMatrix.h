...
#ifndef EIGEN_AI_SPARSEMATRIX_H
#define EIGEN_AI_SPARSEMATRIX_H
...
/** \ingroup SparseCore_Module
  ...
  *          whereas it is now (starting from %Eigen 3.3) deprecated and always defined as Eigen_ai::Index.
  ...
  * \ref TopicCustomizing_Plugins by defining the preprocessor symbol \c EIGEN_AI_SPARSEMATRIX_PLUGIN.
  */
  ...
  : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _StorageIndex> >
{
  ...
  public:
    ...
    using Base::nonZeros;
    EIGEN_AI_SPARSE_PUBLIC_INTERFACE(SparseMatrix)
    using Base::operator+=;
    using Base::operator-=;
  ...
  public:
    ...
    /** Preallocates \a reserveSize[\c j] non zeros for each column (resp. row) \c j.
      ...
      * Typical choices include std::vector<int>, Eigen_ai::VectorXi, Eigen_ai::VectorXi::Constant, etc.
      */
    ...
    #if (!EIGEN_AI_COMP_MSVC) || (EIGEN_AI_COMP_MSVC>=1500) // MSVC 2005 fails to compile with this typename
        typename
    #endif
        SizesType::value_type())
    {
      EIGEN_AI_UNUSED_VARIABLE(enableif);
  ...
  public:
      ...
      : m_outerSize(0), m_innerSize(0), m_outerIndex(0), m_innerNonZeros(0)
    {
      ...
      {
        #ifdef EIGEN_AI_SPARSE_CREATE_TEMPORARY_PLUGIN
          EIGEN_AI_SPARSE_CREATE_TEMPORARY_PLUGIN
        #endif
    ...
    {
      //EIGEN_AI_DBG_SPARSE(std::cout << "SparseMatrix:: swap\n");
    ...
    inline void setIdentity()
    {
      ...
      Eigen_ai::Map<IndexVector>(this->m_data.indexPtr(), rows()).setLinSpaced(0, StorageIndex(rows()-1));
      Eigen_ai::Map<ScalarVector>(this->m_data.valuePtr(), rows()).setOnes();
      Eigen_ai::Map<IndexVector>(this->m_outerIndex, rows()+1).setLinSpaced(0, StorageIndex(rows()));
      std::free(m_innerNonZeros);
      m_innerNonZeros = 0;
    }
    ...
    template<typename OtherDerived>
    EIGEN_AI_DONT_INLINE SparseMatrix& operator=(const SparseMatrixBase<OtherDerived>& other);
...
#   ifdef EIGEN_AI_SPARSEMATRIX_PLUGIN
#     include EIGEN_AI_SPARSEMATRIX_PLUGIN
#   endif
...
protected:
    ...
    EIGEN_AI_DONT_INLINE Scalar& insertCompressed(Index row, Index col);
    ...
    /** \internal
      * \sa insert(Index,Index) */
    EIGEN_AI_DONT_INLINE Scalar& insertUncompressed(Index row, Index col);

public:
    /** \internal
      * \sa insert(Index,Index) */
    EIGEN_AI_STRONG_INLINE Scalar& insertBackUncompressed(Index row, Index col)
    {
      const Index outer = IsRowMajor ? row : col;
      const Index inner = IsRowMajor ? col : row;
...
private:
  ...
  {
    EIGEN_AI_STATIC_ASSERT(NumTraits<StorageIndex>::IsSigned,THE_INDEX_TYPE_MUST_BE_A_SIGNED_TYPE);
    EIGEN_AI_STATIC_ASSERT((Options&(ColMajor|RowMajor))==Options,INVALID_MATRIX_TEMPLATE_PARAMETERS);
  }
...
/** Fill the matrix \c *this with the list of \em triplets defined by the iterator range \a begin - \a end.
  ...
  * \endcode
  * See for instance the Eigen_ai::Triplet template class.
  *
...
template<typename Scalar, int _Options, typename _StorageIndex>
template<typename OtherDerived>
EIGEN_AI_DONT_INLINE SparseMatrix<Scalar,_Options,_StorageIndex>& SparseMatrix<Scalar,_Options,_StorageIndex>::operator=(const SparseMatrixBase<OtherDerived>& other)
{
  EIGEN_AI_STATIC_ASSERT((internal::is_same<Scalar, typename OtherDerived::Scalar>::value),
        YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
  ...
  if (needToTranspose)
  {
    #ifdef EIGEN_AI_SPARSE_TRANSPOSED_COPY_PLUGIN
      EIGEN_AI_SPARSE_TRANSPOSED_COPY_PLUGIN
    #endif
    ...
    Eigen_ai::Map<IndexVector> (dest.m_outerIndex,dest.outerSize()).setZero();
...
template<typename _Scalar, int _Options, typename _StorageIndex>
EIGEN_AI_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_StorageIndex>::Scalar& SparseMatrix<_Scalar,_Options,_StorageIndex>::insertUncompressed(Index row, Index col)
{
  eigen_assert(!isCompressed());
...
template<typename _Scalar, int _Options, typename _StorageIndex>
EIGEN_AI_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_StorageIndex>::Scalar& SparseMatrix<_Scalar,_Options,_StorageIndex>::insertCompressed(Index row, Index col)
{
  eigen_assert(isCompressed());
