...
#ifndef EIGEN_AI_SPARSEUTIL_H
#define EIGEN_AI_SPARSEUTIL_H

namespace Eigen_ai { 

#ifdef NDEBUG
#define EIGEN_AI_DBG_SPARSE(X)
#else
#define EIGEN_AI_DBG_SPARSE(X) X
#endif

#define EIGEN_AI_SPARSE_INHERIT_ASSIGNMENT_OPERATOR(Derived, Op) \
template<typename OtherDerived> \
EIGEN_AI_STRONG_INLINE Derived& operator Op(const Eigen_ai::SparseMatrixBase<OtherDerived>& other) \
{ \
  return Base::operator Op(other.derived()); \
} \
EIGEN_AI_STRONG_INLINE Derived& operator Op(const Derived& other) \
{ \
  return Base::operator Op(other); \
}

#define EIGEN_AI_SPARSE_INHERIT_SCALAR_ASSIGNMENT_OPERATOR(Derived, Op) \
template<typename Other> \
EIGEN_AI_STRONG_INLINE Derived& operator Op(const Other& scalar) \
{ \
  return Base::operator Op(scalar); \
}

#define EIGEN_AI_SPARSE_INHERIT_ASSIGNMENT_OPERATORS(Derived) \
EIGEN_AI_SPARSE_INHERIT_ASSIGNMENT_OPERATOR(Derived, =)


#define EIGEN_AI_SPARSE_PUBLIC_INTERFACE(Derived) \
...
template<typename Lhs, typename Rhs,
         int InnerSize = EIGEN_AI_SIZE_MIN_PREFER_FIXED(internal::traits<Lhs>::ColsAtCompileTime,internal::traits<Rhs>::RowsAtCompileTime)> struct DenseSparseProductReturnType;
         
template<typename Lhs, typename Rhs,
         int InnerSize = EIGEN_AI_SIZE_MIN_PREFER_FIXED(internal::traits<Lhs>::ColsAtCompileTime,internal::traits<Rhs>::RowsAtCompileTime)> struct SparseDenseProductReturnType;
